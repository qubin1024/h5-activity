...
export default {
  ...
  mounted() {
    ...
    const url = baseUrl.apiBaseUrl + "lottery/info";
      ...
      .then(({ data }) => {
        if (data.code == "0") {
          ...
          if (!!data.lottery.discount && data.lottery.discount != null) {
            this.$refs["discount"].activeList = JSON.parse(
              data.lottery.discount
            );
          }
          this.mapInit();
        }
  ...
  methods: {
    ...
    save() {
      ...
      this.$http
        .post(baseUrl.apiBaseUrl + "lottery/save/", a)
        .then(({ data }) => {
