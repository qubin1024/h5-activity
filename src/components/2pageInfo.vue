                            ...
                            <table>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in list" :key="index">
                                        <td><div style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;max-width: 2rem;">{{index+1}}</div></td>
                                        <td><div style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;max-width: 2rem;">{{item.username}}</div></td>
                                        <td><div style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;max-width: 2rem;">{{item.likeNum}}</div></td>
                                        <td><div style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;max-width: 2rem;">{{item.complete_time}}</div></td>
                                    </tr>
                                </tbody>
                            </table>
                        </content-wrap>
...
export default {
    ...
    methods: {
        addActive(){
            ...
            this.$http.post("https://wx.sharkmeida.cn/gather/makeLike", {
        ...
        addZan(){
            ...
            this.$http.post("https://wx.sharkmeida.cn/gather/like", {
                activityId: this.params.id,
        ...
        query() {
            if (!!this.form) {
                ...
                this.ispreview = true;
                this.$refs['bg-main'].style.background = !this.formD.bgImage ? '#192029' : `url(${this.formD.bgImage})`
                this.mapInit();
                return;
            }
            var params = {};
            ...
            this.$http.post("https://wx.sharkmeida.cn/gather/queryLike", {activityId: params.id}).then(({
                data
            }) => {
                if(data.code == 0){
            ...
            this.loading = true
            this.$http.get("https://wx.sharkmeida.cn/gather/info/" + params.id).then(({
                data
            }) => {
                if (data.code == "0") {
                        ...
                        .then((data, status) => {
                            ...
                            wx.ready(function () {
                                ...
                                var shareParam = {
                                    ...
                                    link: "https://wx.sharkmeida.cn/dist/redirect.html?id=" + params.id + "&userid=" + this.user_id + "&shareId=" + this.shareId + "&hash=2pageInfo", // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
