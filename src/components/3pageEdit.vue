    ...
    <div>
        <div class="bg-main" ref='bg-main' v-show="!ispreview">
            ...
            <content-wrap title="奖品信息">
                ...
                <span style="display: block;font-size: 0.3rem;color: #ccc;padding: 0 15px;">参考公式： （原价-底价）/ 大致帮忙减人数=帮减范围平均数，帮减范围平均数-5=最小值，帮减范围平均数+5=最大值。建议设置30-40人帮减即可减至底价</span>
                <x-input title="奖品" v-model="formD.gift" style="font-size: 0.4rem;" :show-clear="showClear"></x-input>
...
export default {
        ...
        this.$http.get(url).then(({
            ...
            if (data.code == "0") {
                ...
                if (data.bargin.thumbnail != null && data.bargin.thumbnail != '') {
                    this.image = "<img src='" + data.bargin.thumbnail + "' style='width: 100%; height: 100%;'/>"
                }
                if (data.bargin.headImage != null && data.bargin.headImage != '') {
                    this.headImage = "<img src='" + data.bargin.headImage + "' style='width: 100%; height: 100%;'/>"
                }
    ...
    methods: {
        ...
        save() {
            ...
            this.$http.post('https://wx.sharkmeida.cn/bargin/save/', a).then(({
                data
            }) => {
